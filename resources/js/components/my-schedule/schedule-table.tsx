        ...
        <div className="overflow-x-auto rounded-md border">
            <table className="w-full text-sm">
                <thead className="bg-muted/50 text-left">
                    <tr>
                        <th className="px-3 py-2">{t('my_schedule.col_date')}</th>
                        <th className="px-3 py-2">{t('my_schedule.col_shift')}</th>
                        <th className="px-3 py-2">{t('my_schedule.col_time')}</th>
                        <th className="px-3 py-2">{t('my_schedule.col_status')}</th>
                    </tr>
                </thead>
                <tbody>
                    {days.map((day) => {
                        ...
                        return (
                            <tr
                                ...
                                className={cn(
                                    ...
                                    day.is_holiday && !isToday && !day.leave && 'bg-muted/30',
                                )}
                            >
                                <td className="px-3 py-2">
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        {isToday ? (
                                            <Badge variant="outline">{t('my_schedule.today')}</Badge>
                                        ) : null}
                                    </div>
                                </td>
                                <td className="px-3 py-2 font-medium">{day.shift_name}</td>
